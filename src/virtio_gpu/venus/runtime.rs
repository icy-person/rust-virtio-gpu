    ...
    pub fn dispatch(&mut self, request: &[u8]) -> Result<VenusResponse, VenusRuntimeError> {
        ...
        match header.typ {
            ...
            CMD_CTX_CREATE => {
                ...
                self.state.create_context(req.header.ctx_id, CAPSET_VENUS, &req.debug_name[..name_len])?;
                self.backend.create_context(req.header.ctx_id, req.context_init, std::str::from_utf8(&req.debug_name[..name_len]).ok())?;
                Ok(Self::ok(header))
            }
            CMD_CTX_DESTROY => {
            ...
            CMD_RESOURCE_CREATE_BLOB => {
                ...
                let entry_bytes = (req.nr_entries as usize).checked_mul(16).ok_or(VenusDispatchError::InvalidRequest)?;
                let end = ResourceCreateBlob::SIZE.checked_add(entry_bytes).ok_or(VenusDispatchError::InvalidRequest)?;
                if request.len() < end { return Err(VenusDispatchError::InvalidRequest.into()); }
                let guest_size = request[ResourceCreateBlob::SIZE..end].chunks_exact(16).map(|entry| u64::from(u32::from_le_bytes(entry[8..12].try_into().expect("fixed-size slice")))).sum();
            ...
            CMD_RESOURCE_MAP_BLOB => {
                ...
                self.state.resources.get_mut(&req.resource_id).ok_or(VenusDispatchError::State(super::VenusStateError::InvalidResource))?.map(req.offset)?;
            ...
            CMD_RESOURCE_ASSIGN_UUID => {
                ...
                let mut uuid = [0u8; 16];
                for chunk in uuid.chunks_exact_mut(8) { x ^= x >> 12; x ^= x << 25; x ^= x >> 27; x = x.wrapping_mul(0x2545_f491_4f6c_dd1d); chunk.copy_from_slice(&x.to_le_bytes()); }
            ...
            CMD_SUBMIT_3D => {
                ...
                let fence_bytes = (req.num_in_fences as usize).checked_mul(8).ok_or(VenusDispatchError::InvalidRequest)?;
                let command_begin = Submit3D::SIZE.checked_add(fence_bytes).ok_or(VenusDispatchError::InvalidRequest)?;
                let command_end = command_begin.checked_add(req.size as usize).ok_or(VenusDispatchError::InvalidRequest)?;
                if command_end > request.len() || req.size % 4 != 0 { return Err(VenusDispatchError::InvalidRequest.into()); }
                let mut in_fences = Vec::with_capacity(req.num_in_fences as usize);
                for i in 0..req.num_in_fences as usize { let start = Submit3D::SIZE + i * 8; in_fences.push(u64::from_le_bytes(request[start..start+8].try_into().expect("fixed-size slice"))); }
                let ring = if req.header.flags & FLAG_INFO_RING_IDX != 0 { req.header.ring_idx } else { 0 };
                let point = self.state.submit(req.header.ctx_id, ring, &in_fences, &request[command_begin..command_end])?;
                let mut commands = request[command_begin..command_end].to_vec();
                self.backend.submit(req.header.ctx_id, req.header.flags, ring, req.header.fence_id, &mut commands, &in_fences)?;
