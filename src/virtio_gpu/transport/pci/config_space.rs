use super::{PciBar, VirtioPciCap64, VirtioPciCapability, VirtioPciNotifyCapability};

/// Minimal conventional PCI configuration-space model used by the VirtIO PCI
/// transport implementation. It deliberately keeps only fields needed by a
/// VirtIO device and its capability list.
...
impl PciConfigSpace {
    ...
    pub fn install_capabilities(
        ...
        for (index, ((offset, len), data)) in locations.iter().zip(encoded.iter()).enumerate() {
            let next = locations.get(index + 1).map(|(next, _)| *next as u8).unwrap_or(0);
    ...
    pub fn validate_bar(&self, bar: PciBar) -> bool {
        self.bar(bar.index)
            .is_some_and(|base| base == bar.base)
    }
}
...
mod tests {
    ...
    fn capability_chain_is_linked() {
        ...
        assert_eq!(config.read(first + 3, 1), Some(VIRTIO_PCI_CAP_COMMON_CFG as u64));
        let next = config.read(first + 1, 1).unwrap() as usize;
        assert_ne!(next, 0);
        assert_eq!(config.read(next + 3, 1), Some(VIRTIO_PCI_CAP_SHARED_MEMORY_CFG as u64));
